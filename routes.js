...
router.post('/cadastrar', (req, res) => {
    const { id, nome, quantidade, preco_unitario } = req.body;

    if (!nome || !quantidade || !preco_unitario) {
        return res.status(400).json({error: 'Todos os campos sao obrigatorios'});
    }    

    const query = 'INSERT INTO produtos (id, nome, quantidade, preco_unitario) VALUES (?, ?, ?, ?)';
    mysqlConnection.query(query, [id, nome, quantidade, preco_unitario], (err) => {
        if (err) {
...
router.get('/produtos', (req, res) => {
    const query = 'SELECT nome, quantidade, preco_unitario FROM produtos';
